...
export function NexusLogo({ size = "md", showWordmark = true, className }: NexusLogoProps) {
  return (
    <div className={clsx("flex min-w-0 items-center gap-3", className)}>
      <div
        className={clsx(
          "relative grid shrink-0 place-items-center overflow-hidden rounded-lg bg-[#12101a] font-bold text-white shadow-[0_0_35px_rgba(200,122,255,0.22)] ring-1 ring-white/10",
          sizeClasses[size]
        )}
        aria-hidden="true"
      >
        <span className="accent-gradient absolute inset-[5px] rounded-md" />
      ...
      {showWordmark ? (
        <div className="min-w-0">
          <p className="font-display accent-text truncate text-base font-bold tracking-normal">ALMAJD</p>
